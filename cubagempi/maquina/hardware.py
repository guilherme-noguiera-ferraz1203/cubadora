...
class Hardware:
    ...
    def _build(self) -> None:
        ...
        if self.simulado:
            with passo("2.0", "Montando simuladores (sem hardware fisico)"):
            ...
            with passo("2.1", f"Abrindo RS-485 ({cfg.rs485.serial_port} + pino DE GPIO{cfg.rs485.de_pin_bcm})"):
                self.rs485 = create_rs485(cfg.rs485, real=True)
            with passo("2.2", f"Abrindo barramento I2C (bus {cfg.balanca.i2c_bus})"):
                self.i2c = create_i2c(cfg.balanca.i2c_bus, real=True)

        with passo("2.3", "Criando sensor ultrassonico + Modbus + CLP"):
        ...
        with passo("2.4", "Iniciando camera"):
            self.camera = create_camera(cfg.camera, self.simulado)

        with passo("2.5", f"Conectando balanca via {'ATmega/I2C' if modelo in ESTATICOS else 'Modbus/485'}"):
        ...
        with passo("2.6", "Configurando LED de status (GPIO6)"):
    ...
    def init(self) -> None:
        ...
        with passo("3.1", "Lendo versoes dos sensores ultrassonicos (trafego RS-485)"):
            try:
